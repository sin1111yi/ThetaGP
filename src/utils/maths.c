...
// Thanks for ledvinap for making such accuracy possible! See:
...
#else
#define sinPolyCoef3                                                           \
  -1.666665710e-1f // Double: -1.666665709650470145824129400050267289858e-1
#define sinPolyCoef5                                                           \
  8.333017292e-3f // Double:  8.333017291562218127986291618761571373087e-3
#define sinPolyCoef7                                                           \
  -1.980661520e-4f // Double: -1.980661520135080504411629636078917643846e-4
#define sinPolyCoef9                                                           \
  2.600054768e-6f // Double:  2.600054767890361277123254766503271638682e-6
#endif

float sin_approx(float x) {
  // Wrap angle to 2π with range [-π π]
  x = fmodf(x, 2.0f * M_PIf);
  // TO DO: these 'while' functions are not put into ITCM ram with current
  // compiler flags A simpler 'if' function works, but gets put into ITCM ram,
  // the extra 4% overflows F7xx ITCM The while function is retained only to
  // avoid ITCM overflow for now ideally we should use the most efficient
  // method, since sin_approx is used a LOT if (x <= -M_PIf) x += 2.0f * M_PIf;
  ...
  // Use axis symmetry around x = ±π/2 for polynomial outside of range [-π/2
  // π/2]
  if (x > M_PIf / 2) {
    x = M_PIf - x; // Reflect
  } else if (x < -M_PIf / 2) {
...
// atan2_approx maximum absolute error = 7.152557e-07 rads (4.098114e-05
// degree)
