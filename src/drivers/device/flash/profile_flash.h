...
static constexpr uint32_t PROFILE_JSON_MAX = 2048;      /**< Max JSON body size per profile */
static constexpr uint16_t PROFILE_MAX_ID   = 15;         /**< Maximum profile ID (0-15) */
static constexpr uint16_t PROFILE_ID_NONE = 0xFFFF;      /**< Sentinel for empty AddressEntry */

static constexpr uint32_t BOOTMETA_BASE  = 0x000000;     /**< BootMeta Ring base */
static constexpr uint32_t BOOTMETA_SIZE  = 0x000400;     /**< BootMeta Ring size (1024 bytes) */
static constexpr uint32_t ADDR_RING_BASE = 0x000400;     /**< Profile Address Ring base */
static constexpr uint32_t ADDR_RING_SIZE = 0x000400;     /**< Profile Address Ring size (1024 bytes) */
static constexpr uint32_t PROFILE0_ADDR  = 0x000800;     /**< Profile 0 primary copy */
static constexpr uint32_t PROFILE0_BACKUP = 0x001000;    /**< Sector 1: Profile 0 backup */
static constexpr uint32_t USER_RING_BASE = 0x002000;     /**< User Ring start address */
...
public:
  ...
  /** Scan BootMeta and Address Rings, determine active profile and next write addr. */
  bool init();
  ...
  /** Load the currently active profile into s_config. */
  bool loadActive(uint8_t *buf, uint16_t *len);
